...
# Budget activities with categories
BUDGET_ITEMS = [
    # Category 1
    ...
    {"name": "2401 fuel or 2103 public transport", "is_category": False},
    {"name": "1204 Subsistence allowance", "is_category": False},
...
with app.app_context():
    # Recreate tables to include new column
    db.drop_all()
    db.create_all()
    
    # Add budget items with categories
    ...
    db.session.commit()
    print(f"Successfully added {len(BUDGET_ITEMS)} budget items with categories!")
    
    # Display added items
    print("\nBudget items with categories:")
    for i, item in enumerate(BUDGET_ITEMS, 1):
        category_marker = " [CATEGORY]" if item["is_category"] else ""
        print(f"{i:2d}. {item['name']}{category_marker}")